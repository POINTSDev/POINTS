...
git clone https://github.com/dzimbeck/POINTS.git
cd POINTS
chmod +x linuxPOINTS.sh
./linuxPOINTS.sh --build

Linux x86_64:

git clone https://github.com/dzimbeck/POINTS.git
cd POINTS
chmod +x linuxPOINTS64.sh
./linux64POINTS.sh --build

Options:
--reset: update dependencies, clone POINTS repository
--force: if you are using other versions such as Debian version > 7

Example with options:
./linux64POINTS.sh --reset --force --build

After successful build, POINTSd can be found in POINTS/src/
